...
**Step B2 - Dispatch ALL subagents using the Agent tool (Qoder)**

> **Qoder platform:** Uses the **Agent (Task) tool** to dispatch subagents for parallel extraction.
...
Accumulate nodes/edges/hyperedges across all results and write to `graphify-out/.graphify_semantic_new.json`.
...
## For native Qoder integration

Run once per project to make graphify always-on in Qoder sessions:

```bash
graphify qoder install
```

This writes a graphify rule to `.qoder/rules/graphify.md` and registers a PreToolUse hook in `.qoder/settings.json`. Both instruct Qoder to check the graph before answering codebase questions and rebuild it after code changes. No manual `/graphify` needed in future sessions.

> **Note:** Unlike AGENTS.md-only platforms, Qoder supports PreToolUse hooks natively. The hook fires automatically before every tool call and injects a reminder when `graphify-out/graph.json` exists.

```bash
graphify qoder uninstall   # remove rule + hook
```
